    ...
    public class Loadout
    {
        public void Load()
        {
			...
			foreach (EquipData equip in agentList)
			{
                ...
                if (model == null)
                {
					...
					EquipmentTypeInfo typeInfo = EquipmentTypeList.instance.GetData(equip.equipmentId);
			...
			foreach (EquipData equip in remainEquip)
			{
				...
				EquipmentModel model = equipList.Find((EquipmentModel x) => x.metaInfo.id == equip.equipmentId && x.owner == null);
                if (model == null)
                {
                    str += "Enf!";
					continue;
        ...
        public void Save()
        {
			...
			foreach (AgentModel agent in AgentManager.instance.GetAgentList())
			{
                ...
                if (weapon != null && weapon.metaInfo.id != 1)
				{
                    str += agent.instanceId + "," + weapon.metaInfo.id + "\n";
					agentList.Add(new EquipData(agent.instanceId, agent.name, weapon.instanceId, weapon.metaInfo.id));
				}
                if (armor != null && armor.metaInfo.id != 22)
				{
                    str += agent.instanceId + "," + armor.metaInfo.id + "\n";
					agentList.Add(new EquipData(agent.instanceId, agent.name, armor.instanceId, armor.metaInfo.id));
				}
			}
