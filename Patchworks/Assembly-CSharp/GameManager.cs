...
public class GameManager : MonoBehaviour
{
    ...
    { // <Mod>
			...
			SpecialModeConfig.instance.GetValue<bool>("RevealOrdeals"))
		{
			...
			{
				string id = "";
				if (ordeal is BugOrdeal) {id = "bug"; } else
				if (ordeal is CircusOrdeal) {id = "circus"; } else
				if (ordeal is MachineOrdeal) {id = "machine"; } else
				if (ordeal is OutterGodOrdeal) {id = "outtergod"; } else
				if (ordeal is ScavengerOrdeal) {id = "scavenger"; } else
				if (ordeal is FixerOrdeal) {id = "fixer"; }
				id += "_";
				switch (ordeal.level)
				{
		...
		text = "Overload Colors:";
        foreach (Color color in UIColorManager.instance.OverloadColor)
		{
			text += "\n";
			if (color == null)
			{
			...
			text += "<color=#" + htmlColor + ">" + htmlColor + "</color>";
		}
    ...
    { // <Mod>
        ...
        if (this.state == GameState.PLAYING)
        {
            ...
            if (SpecialModeConfig.instance.GetValue<bool>("AutoQliphoth"))
            {
                autoQliphothTime += Time.deltaTime;
