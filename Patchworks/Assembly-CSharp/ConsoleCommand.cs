...
public class ConsoleCommand
{
	...
	public void StandardCommandOperation(int index, params object[] param)
	{
		...
		case 7:
			...
			break;
		case 8:
			this.GenerateEquipment((int)float.Parse((string)param[0]), (int)param[1]);
			break;
		case 9:
			if (param.Length >= 2)
			{
		...
		case 32:
			...
			foreach (EquipmentModel equipment in inventory.GetAllEquipmentList())
			{
	...
	{ // <Mod>
		...
		else
		{
			for (int i = 0; i < -count; i++)
			{
				EquipmentModel equipment = InventoryModel.Instance.GetEquipment((long)id);
	...
	public void SwapAbnormalities(CreatureModel creatureModel, CreatureModel creatureModel2)
	{
		...
		if (creatureModel.metadataId == 100020L || creatureModel2.metadataId == 100020L)
		{
