...
public class AgentInfoWindow : MonoBehaviour, IObserver, IDeployResetCalled
{
    ...
    public class UIComponent
    {
        ...
        { // <Mod> Base stats are now displayed; also fixed Ws, As, and Ms stat tooltips so that they show the correct number instead of all saying stat * 0.2%;
    ...
    public class InGameModeComponent
    {
        ...
        { // <Mod> resistances will now use 2 decimal points; fixed Ws, As, and Ms stat tooltips so that they show the correct number instead of all saying stat * 0.2%
            this.AgentTitle.enabled = true;
            this.GradeImage.sprite = DeployUI.GetAgentGradeSprite(agent);
            this.AgentName.text = agent.GetUnitName();
