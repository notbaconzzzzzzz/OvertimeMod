...
public class OneBadManyGoodGift : EquipmentScriptBase // changed from internal
{
	...
	public override float GetWorkProbSpecialBonus(UnitModel actor, SkillTypeInfo skill)
	{
		...
		{
			if (agent.currentSkill.targetCreature.metadataId == 100009L)
			{
				return 10f;
			}
		}
		...
		{
			if (CommandWindow.CommandWindow.CurrentWindow.CurrentWindowType == CommandType.Management && ((CreatureModel)CommandWindow.CommandWindow.CurrentWindow.CurrentTarget).metadataId == 100009L)
			{
				return 10f;
			}
		}
