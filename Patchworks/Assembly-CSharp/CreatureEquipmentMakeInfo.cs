...
public class CreatureEquipmentMakeInfo
{
	...
	public int GiftMultiplierBracket()
	{
		CreatureObserveInfoModel observeInfo = CreatureManager.instance.GetObserve(creature.id);
	...
	public int GiftRiskLevel
	{
		get
		{
			...
			int risk = 1;
			if (EquipmentTypeInfo.BossIds.Contains(equipTypeInfo.id)) {
				risk = 6;
			} else {
				if (creature != null) {
					if (creature.id == 100064L) {
						risk = 5;
					} else {
