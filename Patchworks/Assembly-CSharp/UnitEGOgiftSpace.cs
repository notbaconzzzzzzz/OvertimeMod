...
public class UnitEGOgiftSpace
{
	...
	{ // <Mod> allow EGO gifts to stack even if you occupy the same slot, but not if they're exclusive to each other
		int stackingMax = GetStackingMax();
        if (stackingMax != 1)
        {
			long[] exclude = null;
            foreach (long[] group in exclusiveGifts)
            {
                if (group.Contains(model.metaInfo.id))
				{
					exclude = group;
					break;
				}
			...
			if (exclude != null)
			{
				EGOgiftModel egogiftModel = (model.metaInfo.attachType == EGOgiftAttachType.REPLACE ? this.replacedGifts : this.addedGifts).Find((EGOgiftModel x) => exclude.Contains(x.metaInfo.id));
	...
	public static readonly long[][] exclusiveGifts = new long[][]
	{
		new long[] {
            4000371L,
            4000372L,
