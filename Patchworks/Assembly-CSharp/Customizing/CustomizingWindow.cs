/*
private void ReviseOpenAction(AgentModel agent) // Recustomizing
private void Update() // Copy Paste
public void Confirm() // Recustomizing
private int GetCost() // Customizing appearence is now free
...
namespace Customizing
{
	...
	public class CustomizingWindow : MonoBehaviour
	{
		...
		{ // <Mod>
			...
			CurrentData.agentName.isCustom = true;
			CurrentData.CustomName = agent._agentName.GetName();
		...
		private int GetCost()
		{ // <Mod> customize appearence is now free
			int result;
			try
			{
		...
		public bool PasteAppearanceFromClipboard()
		{
			...
			try
			{
				...
				{
					appearanceUI.NameInput.text = sections[num];
					appearanceUI.OnSetNametext(sections[num]);
					num++;
				}
				try
				{
